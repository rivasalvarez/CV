...
Image Image::nonMax(int window_size){
	...
	for(int y = 0; y < height; y++){
	   ...
	   e = 1 - s;
	      for(j = s; j < e; j++){
		     for(i = s; i < e; i++){
			    if( (j+y >= 0) && (j+y < height) && (x+i > 0) && (x+i < width) &&(data[j+y][i+x] > max) ){
				max = data[j+y][i+x];
				flag = (i==0 && j==0);
			    }
		     }
          }
		     if(!flag && (j+y >= 0) && (j+y < height) && (x+i > 0) && (x+i < width) ){
			 data[y][x] = 0.f;
		     }
	    }
    }
...
Image Image::nonMin(int window_size){
	...
	for(int y = 0; y < height; y++){
	   ...
	   e = 1 - s;
		for(j = s; j < e; j++){
		   for(i = s; i < e; i++) {
			 if( (j+y >= 0) && (j+y < height) && (x+i > 0) && (x+i < width) &&(data[j+y][i+x] < min) ){
			 min = data[j+y][i+x];
			 flag = (i==0 && j==0);
			 }
		   }}
